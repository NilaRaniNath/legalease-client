...
export default function LawyerProfileForm({ userId, initialData, userEmail }) {
  ...
  const saveMutation = useMutation({
    mutationFn: (payload) => lawyerApi.saveProfile(payload),
    onSuccess: (result) => {
    ...
    onError: () => Swal.fire({ icon: "error", title: "Connection failed" }),
  });
  ...
  const createMutation = useMutation({
      ...
      if (result.data.success) {
        ...
        Swal.fire({
          ...
          if (res.isConfirmed) {
            ...
            checkoutForm.action = `/api/checkout_sessions?email=${encodeURIComponent(payload.email)}`;
    ...
    onError: () => Swal.fire({ icon: "error", title: "Server error during registration" }),
  });

  const publishMutation = useMutation({
    mutationFn: (isPublished) =>
      lawyerApi.saveProfile({ userId, isPublished }),
    onSuccess: (result, isPublished) => {
      if (result.data.success) {
        Swal.fire({
          icon: "success",
          title: isPublished ? "Profile Published Live!" : "Profile Hidden (Unpublished)",
          toast: true,
          position: "top-end",
    ...
    onError: () => console.error("Failed to toggle publish status"),
  });
  ...
  const deleteMutation = useMutation({
    ...
    onError: () => Swal.fire("Error!", "Could not delete profile at this time.", "error"),
  });
    ...
    <div className="w-full max-w-xl mx-auto px-4 py-4">
      <Card className="bg-[#152238] border border-slate-700 p-4 sm:p-6 w-full space-y-5 shadow-2xl rounded-2xl">
        ...
        <div className="flex flex-col items-center gap-3">
          ...
          {initialData && (
            <div className="grid grid-cols-2 gap-3 w-full max-w-xs mt-1">
              <Button size="sm" variant="flat" color={profile.isPublished ? "warning" : "success"} onClick={togglePublishStatus} className="w-full text-xs" isLoading={publishMutation.isPending}>
                {!publishMutation.isPending && (profile.isPublished ? <><EyeOff className="w-3.5 h-3.5 mr-1" /> Unpublish</> : <><Eye className="w-3.5 h-3.5 mr-1" /> Publish Live</>)}
              </Button>
              <Button size="sm" variant="flat" color="danger" onClick={handleDeleteProfile} className="w-full text-xs" isLoading={deleteMutation.isPending}>
                {!deleteMutation.isPending && <><Trash2 className="w-3.5 h-3.5 mr-1" /> Delete Profile</>}
              </Button>
            </div>
          )}
        </div>
        ...
        <form onSubmit={handleProfileSubmit} className="space-y-4 text-left">
          ...
          >
            {saveMutation.isPending ? "Saving..." : initialData ? "Update Legal Profile" : "Save & Proceed to Pay"}
          </Button>
        </form>
      </Card>
    </div>
